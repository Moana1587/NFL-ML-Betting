...
data_url = 'https://stats.nfl.com/api/teams/seasonal-stats?' \
           'season=2024&seasonType=REG&conference=&division=&format=json'
...
def createTodaysGames(games, df, odds):
    ...
    for game in games:
        ...
        schedule_df = pd.read_csv('Data/nfl-2024-UTC.csv', parse_dates=['Date'], date_format='%d/%m/%Y %H:%M')
...
def main():
    ...
    try:
        ...
        if df.empty:
            print("No data available from API. Using mock data for demonstration.")
            # You can load mock data here if needed
            df = pd.DataFrame()  # Empty dataframe for now
